...
public class PostService {
    ...
    // boardCode를 파라미터로 받도록 수정
    public ListData<Post> selectPostList(int reqPage, String boardCode) {
        Connection conn = JDBCTemplate.getConnection();
        int numPerPage = 10; // 페이지당 게시글 수 (기존 viewPostCnt)
        int end = reqPage * numPerPage;
        int start = end - numPerPage + 1;
        
        // DAO 호출 시 boardCode 전달
        ArrayList<Post> list = dao.selectPostList(conn, start, end, boardCode);
        
        // 전체 게시글 수 조회 시 boardCode 전달
        ...
        String url = "getList?boardCode=" + boardCode + "&reqPage="; // 기본 URL (컨텍스트 경로는 JSP에서 추가)

        if (pageNo != 1) {
            pageNavi += "<li><a class='page-item' href='" + url + (pageNo - 1) + "'>";
            pageNavi += "<span class='material-icons'>chevron_left</span></a></li>";
        }
        for (int i = 0; i < pageNaviSize; i++) {
            if (pageNo == reqPage) {
                pageNavi += "<li><a class='page-item active-page' href='" + url + pageNo + "'>" + pageNo + "</a></li>";
            } else {
                pageNavi += "<li><a class='page-item' href='" + url + pageNo + "'>" + pageNo + "</a></li>";
            }
            pageNo++;
            if (pageNo > totalPage) {
                break;
        ...
        if (pageNo <= totalPage) {
            pageNavi += "<li><a class='page-item' href='" + url + pageNo + "'>";
